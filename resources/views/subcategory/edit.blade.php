                ...
                <form action="{{ route('subcategory.update', $subcategory->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        <label for="status"
                            ...
                            <input type="radio" name="status" value="{{ App\Models\SubCategory::STATUS_ACTIVE }}"
                            @if (!is_null(old('status')) && old('status') == App\Models\SubCategory::STATUS_ACTIVE) checked @elseif($subcategory->status && !old('status')) checked @endif class="status" /> Active
                     <input class="status" type="radio" name="status" value="{{ App\Models\SubCategory::STATUS_INACTIVE }}"
                            @if (!is_null(old('status')) && old('status') == App\Models\SubCategory::STATUS_INACTIVE) checked @elseif(!$subcategory->status && !old('status')) checked @endif /> In-Active
